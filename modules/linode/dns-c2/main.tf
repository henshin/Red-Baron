...
resource "null_resource" "ansible_provisioner" {
  ...
  depends_on = ["linode_linode.dns-c2"]

  triggers {
    droplet_creation = "${join("," , linode_linode.dns-c2.*.id)}"
    policy_sha1 = "${sha1(file(var.ansible_playbook))}"
  }

  provisioner "local-exec" {
    command = "ansible-playbook ${join(" ", compact(var.ansible_arguments))} --user=root --private-key=./data/ssh_keys/${linode_linode.dns-c2.*.ip_address[count.index]} -e host=${linode_linode.dns-c2.*.ip_address[count.index]} ${var.ansible_playbook}"
...
data "template_file" "ssh_config" {
  ...
  vars {
    name = "dns_c2_${linode_linode.dns-c2.*.ip_address[count.index]}"
    hostname = "${linode_linode.dns-c2.*.ip_address[count.index]}"
    user = "root"
    identityfile = "${path.root}/data/ssh_keys/${linode_linode.dns-c2.*.ip_address[count.index]}"
  }
